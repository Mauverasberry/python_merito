...
@pytest.mark.skip(reason='Bo tak') #omija 8 testow z parametrize
@pytest.mark.parametrize('username, passwd, url', test_data)


def test_login_page_standard_user(username,url):
...
@pytest.mark.skipif(len('piuesek') ==5, reason= 'piesek jeszcze za maly')
def test1():
    assert 2 + 3 == 5
